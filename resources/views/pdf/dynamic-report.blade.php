...
<head>
    ...
    <style>
        ...
        header { position: fixed; top: -90px; left: 0; right: 0; height: 100px; }
        body { font-family: "Times New Roman", Times, serif; font-size: 8pt; margin: 0; padding: 0; }
        ...
        .main-table th { background-color: #ffff00; font-size: 7pt; line-height: 1.1; text-transform: uppercase; }

        /* Style Baru untuk Lampiran Foto */
        .photo-section { page-break-before: always; margin-top: 20px; }
        ...
        .photo-img { width: 100%; height: 160px; object-fit: cover; border-bottom: 1px solid #000; }
        .photo-caption { padding: 5px; text-align: left; font-size: 7pt; line-height: 1.2; }
        ...
        .legend-table td, .legend-table th { border: 1px solid black; padding: 3px; }
    </style>
</head>
<body>
    ...
    <header>
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="border-bottom: 2px solid #999; width: 15%; text-align: center;"><img src="{{ public_path('images/logo-msm.png') }}" width="60"></td>
                ...
                <td style="border-bottom: 2px solid #999; width: 15%; text-align: center;"><img src="{{ public_path('images/logo-archi.png') }}" width="60"></td>
            </tr>
        </table>
    </header>
    ...
    <main>
            ...
            <tbody>
                @foreach ($data as $index => $item)
                    <tr>
                        ...
                        @foreach ($structure['inputs'] as $input) <td>{{ $item->conditions[$input] ?? '-' }}</td> @endforeach
                        @foreach ($structure['checks'] as $check)
                            <td>
                                @php $val = $item->conditions[$check] ?? null; @endphp
                                @if ($val === true || $val === 'true' || $val === 1) <span class="good">✔</span>
                                @elseif($val === false || $val === 'false' || $val === 0) <span class="nogood">✘</span>
                                @else - @endif
                            </td>
                        @endforeach
                        ...
                        <td>
                            @php
                                ...
                                $initials = collect($names)->map(function ($n) {
                                    return collect(preg_split('/[\s,]+/', trim($n)))->map(fn($w) => strtoupper(substr($w, 0, 1)))->implode('');
                                })->implode(', ');
                            @endphp
                            {{ $initials }}
                        </td>
        ...
        <div class="photo-section">
            <div style="background-color: #eee; padding: 5px; text-align: center; border: 1px solid #000; margin-bottom: 15px;">
            ...
            <table class="no-border" style="width: 100%;">
                <tr>
                    ...
                    <td class="no-border" style="width: 30%; vertical-align: top;">
                        <table class="legend-table">
                            @php
                                $uniqueNames = collect($data)->pluck('inspected_by')->flatMap(fn($item) => explode('|', $item))->map(fn($name) => trim($name))->unique()->filter();
                            @endphp
