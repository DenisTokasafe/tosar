<section class="w-full">
    ...
    <x-tabs-wpi.layout>
        ...
        <div class="relative overflow-hidden border rounded-lg shadow-inner bg-slate-50">
            @php
            // INPUTS tetap diambil global per type untuk header
            $inputs = $fields[$type]['inputs'] ?? [];

            // Ambil data pertama untuk referensi technical data
            $firstEquipment = $allMasterData->first();
            $techKeys = $firstEquipment && is_array($firstEquipment->technical_data)
            ? array_keys($firstEquipment->technical_data)
            : [];

            /* HEADER DYNAMICS:
            Karena setiap baris bisa beda kolom, kita ambil struktur 'checks'
            dari masterChecklists pertama yang tersedia untuk mengisi Header Tabel.
            */
            $firstMasterId = $firstEquipment ? $firstEquipment->id : null;
            $headerChecks = $firstMasterId ? ($masterChecklists[$firstMasterId] ?? []) : [];
            @endphp
                ...
                <table class="table border-separate md:table-fixed table-xs table-pin-rows border-spacing-0">
                    <thead>
                        <tr class="capitalize bg-slate-100 text-slate-700">
                            ...
                            <th class="border-b border-r bg-slate-100 text-[10px]">
                                @if(is_array($techKey))
                                {{ $techKey['label'] ?? 'N/A' }}
                                @else
                                {{ $techKey }}
                            ...
                            {{-- Header untuk Kolom Checklist --}}
                            @foreach ($headerChecks as $checkItem)
                    ...
                    <tbody>
                        ...
                        <tr class="transition-colors hover:bg-blue-50/30">
                            ...
                            {{-- IMPLEMENTASI LOGIKA DINAMIS PER BARIS --}}
                            @php
                            // Ambil checklist khusus ID master ini (dari controller)
                            $rowChecks = $masterChecklists[$master->id] ?? [];
                            @endphp

                            @foreach ($rowChecks as $field)
                            @php
                            ...
                            <td class="p-1 text-center bg-white border-b border-r">
                                @if ($fieldType === 'text')
                                {{-- Tampilan Input Text (SCBA BAR, Oksigen PSI, dll) --}}
                                <input type="text"
                            ...
                            <td class="p-2 text-center bg-white border-b" wire:loading.add.class="skeleton" wire:target="dokumentasi.{{ $master->id }}">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <div class="tooltip md:tooltip-left">
                                        <div class="z-40 tooltip-content text-[10px]">upload foto</div>
        ...
        {{-- Footer area (Pemeriksa & Simpan) tetap sama --}}
        <div class="flex flex-col items-start justify-between gap-4 pt-2 mt-2 border-t md:flex-row">
            <div class="w-full md:max-w-md">
                ...
                <div class="mt-2 position-relative d-inline-block">
                    ...
                    <button type="button" wire:click="removeFotoArea" class="top-0 btn btn-danger btn-sm position-absolute end-0">&times;</button>
                </div>
                @endif
            </div>
            <div class="w-full md:max-w-md">
