{{-- Container Utama dengan Alpine.js untuk Persistence --}}
<div x-data="{
    /* Mengikat property Livewire ke LocalStorage browser */
    draftDescription: $persist(@entangle('description')),
    draftEmergency: $persist(@entangle('emergency_action')),
    draftDamage: $persist(@entangle('damage_detail')),
    draftLocationSpecific: $persist(@entangle('location_specific')),
}"
    {{-- Listener untuk membersihkan storage setelah sukses simpan --}}
    x-on:clear-draft.window="localStorage.clear()">
    ...
    <div class="grid grid-cols-1 gap-2 mb-8 md:grid-cols-2 lg:grid-cols-3">
        ...
        @if ($location_id)
        {{-- Menggunakan x-model untuk sinkronisasi ke persist --}}
        <x-form.input-text label="Lokasi Spesifik" model="location_specific" x-model="draftLocationSpecific" placeholder="Masukkan detail lokasi spesifik..." required />
        @endif
    ...
    {{-- Integrasi Risk Matrix (Tetap menggunakan wire:model karena logika kompleks di backend) --}}
    <fieldset class="p-3 my-4 border shadow-md border-base-300 fieldset card bg-base-100">
                ...
                <table class="table table-xs w-60">
                    ...
                    <tbody>
                        ...
                        <tr class="w-32 text-xs text-center">
                            ...
                            @endphp
                            <td class="border cursor-pointer @if ($likelihood_id == $l->id && $consequence_id == $c->id) border-2 bg-primary border-primary-content @endif">
                                <span wire:click="edit({{ $l->id }}, {{ $c->id }})" class="btn btn-square btn-xs {{ $color }}">
        ...
        <table class="table mt-4 table-xs table-zebra">
            <tr>
                ...
                <td class="pl-2 text-xs border-base-300">{{ $RiskAssessment->name }}</td>
            </tr>
            <tr>
                <th class="w-40 text-xs border-base-300">Notify</th>
                <td class="pl-2 text-xs border-base-300">{{ $RiskAssessment->reporting_obligation }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="pl-2 text-xs border-base-300">{{ $RiskAssessment->coordinator }}</td>
            </tr>
        </table>
        @endif
    </fieldset>

    {{-- Input Deskripsi dengan Persistence --}}
    <x-form.text_area
        label="Narasi detail mengenai urutan kejadian (5W+1H)"
        model="description"
        x-model="draftDescription"
        placeholder="{{ __('Contoh: Siapa yang terlibat, Apa yang terjadi, Dimana, Kapan, Mengapa, dan Bagaimana urutannya.')}}" required />

    <x-form.text_area
        label="Tindakan Darurat"
        model="emergency_action"
        x-model="draftEmergency"
        placeholder="{{ __('Jelaskan tindakan segera yang dilakukan setelah kejadian...')}}" required />
    ...
    <fieldset class="p-3 my-4 border shadow-md border-base-300 fieldset card bg-base-100">
        ...
        <x-form.text_area
            label="Detail Kerusakan Alat / Lingkungan"
            model="damage_detail"
            x-model="draftDamage"
            placeholder="{{ __('Jelaskan kerusakan alat atau dampak lingkungan...')}}" required />
    </fieldset>
    @endif
</div>