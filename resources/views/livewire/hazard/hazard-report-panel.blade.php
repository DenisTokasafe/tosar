        ...
        <table class="table text-xs border table-xs">
            <thead>
                <tr class="text-center bg-gray-100">
                        ...
                        <ul class="p-2 overflow-y-auto shadow-lg dropdown menu w-52 rounded-box bg-base-100 max-h-60"
                            ...
                            <li>
                                <label class="flex items-center p-1 rounded cursor-pointer hover:bg-gray-100">
                                    <input type="checkbox" wire:model.live="filterEventType" value="{{ $event_type->id }}" class="text-blue-600 rounded form-checkbox">
                                    <span class="ml-2 text-xs capitalize">
                                        {{ __($event_type->event_type_name) }}
                                    </span>
                                </label>
                            </li>
                            @endforeach
                    ...
                    <th class="border">
                        {{ __('Jenis Bahaya') }}
                        <button class="btn btn-ghost btn-xs" popovertarget="eventSubType" style="anchor-name:--eventSubType">...</button>
                        <ul class="p-2 overflow-y-auto shadow-lg dropdown menu w-52 rounded-box bg-base-100 max-h-60"
                            popover id="eventSubType" style="position-anchor:--eventSubType; inset-area: bottom span-right;">
                            @foreach ($filterOptions['EventSubType'] as $event_sub_type)
                            <li>
                                <label class="flex items-center p-1 rounded cursor-pointer hover:bg-gray-100">
                                    <input type="checkbox" wire:model.live="filterEventSubType" value="{{ $event_sub_type->id }}" class="text-blue-600 rounded form-checkbox">
                                    <span class="ml-2 text-xs capitalize">{{ __($event_sub_type->event_sub_type_name) }}</span>
                                </label>
                            </li>
                            @endforeach
                        </ul>
                    ...
                    <th class="border">
                        ...
                        <button class="btn btn-ghost btn-xs" popovertarget="divisi_dept" style="anchor-name:--divisi_dept">...</button>
                        <ul class="p-2 overflow-y-auto shadow-lg dropdown menu w-52 rounded-box bg-base-100 max-h-60"
                            popover id="divisi_dept" style="position-anchor:--divisi_dept; inset-area: bottom span-right;">
                            @foreach ($filterOptions['Department'] as $dept)
                            <li>
                                <label class="flex items-center p-1 rounded cursor-pointer hover:bg-gray-100">
                                    <input type="checkbox" wire:model.live="filterDepartment" value="{{ $dept->id }}" class="text-blue-600 rounded form-checkbox">
                    ...
                    <th class="border">
                        {{ __('Divisi Pelapor') }}
                        <button class="btn btn-ghost btn-xs" popovertarget="popover_reporter_dept" style="anchor-name:--anchor_reporter_dept">...</button>
                        <ul class="p-2 overflow-y-auto shadow-lg dropdown menu w-52 rounded-box bg-base-100 max-h-60"
                            popover id="popover_reporter_dept" style="position-anchor:--anchor_reporter_dept; inset-area: bottom span-right;">
                            @foreach ($filterOptions['ReporterDepartments'] as $deptName)
                            <li>
                                <label class="flex items-center p-1 rounded cursor-pointer hover:bg-gray-100">
                                    <input type="checkbox" wire:model.live="filterReporterDept" value="{{ $deptName }}" class="text-blue-600 rounded form-checkbox">
                            ...
                            <li class="mt-2 border-t">
                                <button wire:click="$set('filterReporterDept', [])" class="w-full text-red-500 btn btn-ghost btn-xs">{{ __('Reset Filter') }}</button>
                            </li>
                            @endif
                        </ul>
                    </th>
                    ...
                    <th class="relative border">
                        {{ __('Status') }}
                        <button class="btn btn-ghost btn-xs" popovertarget="popover-1" style="anchor-name:--anchor-1">...</button>
                        <ul class="shadow-sm dropdown menu w-52 rounded-box bg-base-100" popover id="popover-1" style="position-anchor:--anchor-1">
                            @foreach ($availableStatuses as $status)
                            <li>
                                <label class="flex items-center p-1 mb-1 rounded cursor-pointer hover:bg-gray-100">
                                    <input type="checkbox" wire:model.live="filterStatus" value="{{ $status }}" class="text-blue-600 rounded form-checkbox">
                                    <span class="ml-2 text-xs capitalize">{{ __(str_replace('_', ' ', $status)) }}</span>
                                </label>
                            </li>
                            @endforeach
                        </ul>
                    ...
                    <th class="flex-col text-center border">{{ __('Aksi') }}</th>
                </tr>
            </thead>
            <tbody>
