<section class="w-full">
    ...
    <x-tabs-equipment.layout heading="Inspection Checklist"
        ...
        <div class="md:shadow-md md:px-4 md:mb-10 md:absolute md:inset-x-0 md:top-0 md:z-20 md:flex-row bg-base-100 md:inset-shadow-sm">
            <button class="btn btn-primary btn-soft btn-xs" onclick="checklist_modal.showModal()" wire:click="resetForm">
                + Tambah Checklist
            </button>
        </div>
        ...
        <table class="table mt-4 table-zebra table-xs">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->location_keyword }}</td>
                    <td class="text-xs">
                        <strong>Inputs:</strong> {{ implode(', ', $item->inputs) }} <br>
                        <strong>Requirements:</strong> {{ implode(', ', $item->inputs_req ?? []) }} <br>
        ...
        <dialog id="checklist_modal" class="modal" wire:ignore.self>
            <div class="flex flex-col w-11/12 max-w-4xl p-0 modal-box"> {{-- Lebar ditingkatkan ke max-w-4xl --}}

                <div class="p-6 pb-2">
                    <h3 class="text-lg font-bold">{{ $checklist_id ? 'Edit' : 'Tambah' }} Checklist</h3>
                    <p class="text-sm text-gray-500">Kelola input, persyaratan, dan poin pemeriksaan peralatan.</p>
                </div>
                ...
                <div class="p-6 pt-2 overflow-y-auto max-h-[70vh]">
                    ...
                    {{-- Tampilan Grid 3 Kolom untuk Input Dinamis --}}
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">

                        {{-- 1. INPUTS FIELD --}}
                        <div class="space-y-2">
                            <label class="text-xs font-bold tracking-wider text-gray-400 uppercase">Inputs Field</label>
                            @foreach ($inputs as $index => $value)
                            <div class="flex items-center gap-2 group">
                                <x-form.input-floating label="Input {{ $index + 1 }}" type='text'
                                    model="inputs.{{ $index }}" placeholder="Input {{ $index + 1 }}" />
                            ...
                            @endforeach
                            <button wire:click="addInput" class="no-underline btn btn-ghost btn-xs text-primary hover:bg-primary/10">+ Add Input</button>
                        </div>

                        {{-- 2. INPUTS REQ (BARU) --}}
                        <div class="space-y-2">
                            <label class="text-xs font-bold tracking-wider text-gray-400 uppercase">Requirements</label>
                            @foreach ($inputs_req as $index => $value)
                            <div class="flex items-center gap-2 group">
                                <x-form.input-floating label="Req {{ $index + 1 }}" type='text'
                                    model="inputs_req.{{ $index }}" placeholder="Requirement {{ $index + 1 }}" />
                                <button wire:click="removeInputReq({{ $index }})"
                                    class="transition-opacity opacity-50 btn btn-square btn-xs btn-error btn-outline group-hover:opacity-100">×</button>
                            </div>
                            @endforeach
                            <button wire:click="addInputReq" class="no-underline btn btn-ghost btn-xs text-primary hover:bg-primary/10">+ Add Req</button>
                        </div>

                        {{-- 3. CHECKPOINTS --}}
                        <div class="space-y-2">
                            <label class="text-xs font-bold tracking-wider text-gray-400 uppercase">Checkpoints</label>
                            @foreach ($checks as $index => $value)
                            <div class="flex items-center gap-2 group">
                                <x-form.input-floating label="Check {{ $index + 1 }}" type='text'
                                    model="checks.{{ $index }}" placeholder="Check {{ $index + 1 }}" />
                                <button wire:click="removeCheck({{ $index }})"
                                    class="transition-opacity opacity-50 btn btn-square btn-xs btn-error btn-outline group-hover:opacity-100">×</button>
                            </div>
                            @endforeach
                            <button wire:click="addCheck" class="no-underline btn btn-ghost btn-xs text-primary hover:bg-primary/10">+ Add Check</button>
                        </div>
                    </div>
                </div>
