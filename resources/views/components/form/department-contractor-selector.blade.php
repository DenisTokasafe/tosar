...
<fieldset {{ $attributes->merge(['class' => 'fieldset']) }}>
    ...
    {{-- Dropdown Department --}}
    <div wire:key="dropdown-dept-{{ $model_dept }}" class="{{ $deptCont === 'dept' ? 'block' : 'hidden' }} mt-1">
    ...
    {{-- Dropdown Contractor --}}
    <div wire:key="dropdown-cont-{{ $model_cont }}" class="{{ $deptCont === 'cont' ? 'block' : 'hidden' }} mt-1">
